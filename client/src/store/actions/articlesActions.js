...
export const getMyArticles = () => {
    return dispatch => {
        fetch('http://localhost:5000/api/articles/myarticles', {
            headers: {
                'Authorization': 'Bearer ' + localStorage.getItem('jwtToken'),
                'Content-Type': 'application/json'
            },
...
export const submitNewArticle = (articleData) => {
    return dispatch => {
        return fetch('http://localhost:5000/api/articles/add', options(articleData))
        .then(res => res.json())
    }
};
...
export const saveArticle = (articleId, articleData) => {
    return dispatch => {
        return fetch('http://localhost:5000/api/articles/edit/' + articleId, options(articleData))
        .then(res => res.json())
    }
}
...
export const deleteArticle = (articleId) => {
    return dispatch => {
        return fetch('http://localhost:5000/api/articles/delete/' + articleId, {
            headers: {
                'Authorization': 'Bearer ' + localStorage.getItem('jwtToken'),
                'Content-Type': 'application/json'
            },
